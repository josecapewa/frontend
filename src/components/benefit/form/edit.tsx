...
import { BenefitOtherData } from "./add";

export default function BenefitEditForm({
  onSubmit,
  defaultValues,
  otherData,
}: {
  onSubmit: (data: BenefitToUpdate) => Promise<void>;
  defaultValues?: BenefitToUpdate;
  otherData?: BenefitOtherData;
}) {
  const [isLoading, setIsLoading] = useState<boolean>(false);

  const form = useForm<BenefitToUpdate>({
      resolver: zodResolver(benefitValidations),
      defaultValues,
  });

  const handleOnSubmit = async (data: BenefitToUpdate) => {
    setIsLoading(true);
    toastPromiseConfig({
      fn: onSubmit(data).finally(() => setIsLoading(false)),
      loading: "A eitar benefício...",
      success: "Benefício editado com sucesso.",
    });
  };
