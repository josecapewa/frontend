...
import { z } from "zod";
export type CustomUserToCreate = z.infer<typeof userSchema>;

export default function UserAddForm({
  onSubmit,
}: {
  onSubmit: (data: CustomUserToCreate) => Promise<void>;
}) {
  const [isLoading, setIsLoading] = useState<boolean>(false);

  const form = useForm<CustomUserToCreate>({
  ...
  const handleOnSubmit = async (data: CustomUserToCreate) => {
