...
export function TrainingFieldTotalStudentsChart({
  traningFieldStatus,
  lectiveYear,
}: {
  traningFieldStatus?: TraningFieldTotalStudents[];
  lectiveYear?: string;
}) {
  const id = "pie-interactive";
  const chartData = useMemo(
    () =>
      (Array.isArray(traningFieldStatus) &&
  ...
  const [activeTraningField, setActiveTrainingField] = useState(
    traningFieldStatus && traningFieldStatus.length > 0
  ...
  if (!Array.isArray(traningFieldStatus)) {
    return <TrainingFieldTotalStudentsSkeleton />;
  }
  return (
    <Card data-chart={id} className="flex flex-col">
      ...
      <CardHeader className="flex-col mb-4 md:flex-row items-start gap-2 space-y-0 pb-0">
        <div className="grid gap-1">
          <CardTitle>Total de alunos por área de formação</CardTitle>
...
export function TrainingFieldTotalStudentsSkeleton() {
  return (
